...
export default function NetworkGameCanvas(){
  ...
  useEffect(()=>{networkStateRef.current=networkState;},[networkState]);
  ...
  const statusLabel=networkState.status==='connected'?'SERVER CONNECTED':networkState.status==='connecting'?'CONNECTING…':networkState.status==='error'?'SERVER ERROR':'SERVER OFFLINE',statusDot=networkState.status==='connected'?'bg-green-400':networkState.status==='connecting'?'bg-yellow-400':'bg-red-400';
